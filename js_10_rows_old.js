...
(function f_set_arr_for_G_ROWS() {
    ...
    function f_is_row(xyz_from, xyz_d, len_3_or_4) {
        var xyz_delta = G.ROWS.f_mult_delta(xyz_d, len_3_or_4 - 1);
        ...
        if (len_3_or_4 == 3) {
            ...
            //false, if can be extended
            if (G.ROWS.f_is_xyz_on_cube_board(xyz_pre)) {return false};

            var xyz_last_4 = G.ROWS.f_add_delta(xyz_from, G.ROWS.f_mult_delta(xyz_d, 3));
            //true, if can not be extended
            return (!G.ROWS.f_is_xyz_on_cube_board(xyz_last_4));
        }
        return (len_3_or_4 != 3);
    }

    function f_arr_by_from_and_delta(xyz_from, xyz_delta, len_3_or_4) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        for (j = 0; j < 13; j++) {
            ...
            if (f_is_row(xyz_from, xyz_delta, 4)) {
                G.ROWS.R4.ARR_4_UNIQUE.push(f_arr_by_from_and_delta(xyz_from, xyz_delta, 4));
            }
            if (f_is_row(xyz_from, xyz_delta, 3)) {
                G.ROWS.R3.ARR_3_UNIQUE.push(f_arr_by_from_and_delta(xyz_from, xyz_delta, 3));
            }
        }
    }

    // ***
    G.f_show_row_3 = function(n_row) {
        var r = G.ROWS.R3.ARR_3_UNIQUE[n_row];
        function f_0_or_1(i) {
            function f_check(n02) {
                return (G.ROWS.R4.f_xyz_to_delta(r[n02]) == i);
            }
            if (f_check(0)) {return 3; };
            return (f_check(0) || (f_check(1) || f_check(2)) ? 1 : 2);
        }
        return G.GENERATE.f_array_by_function(f_0_or_1, 64);
    }
    console.log(G.f_show_row_3(0));
    G.f_show_row_3.len = G.ROWS.R3.ARR_3_UNIQUE.length;
    G.f_show_row_3.n = 0;
    // ***
}());